function Chess() {
    ...
    //init
    this.board_init = function() {
        for (j = 1; j <= 8; j++) {
            for (i = 1; i <= 4; i++)
                board.shape_xy("blank", bound_top, bound_left, j, i);
        }

        for (j = 1, i = 8; j <= 8; j++, i--)
            board.shape_xy_test("boundary", bound_top, bound_left, j, 0, STR_NUM[i], 20, "#9C9C9C");
        for (i = 1; i <= 4; i++)
            board.shape_xy_test("boundary", bound_top, bound_left, 9, i, EN_NUM[i], 20, "#9C9C9C");

        board.shape_xy("backslash", bound_top, bound_left, 4, 3);
        board.shape_xy("backslash", bound_top, bound_left, 5, 4);
        board.shape_xy("slash", bound_top, bound_left, 4, 4);
        board.shape_xy("slash", bound_top, bound_left, 5, 3);
    }
