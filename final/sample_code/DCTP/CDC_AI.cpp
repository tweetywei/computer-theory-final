...
void CDC_AI::initBoardState ()
{
	...
	for(int i=0;i<10;i++)
	{
		for(int j=0;j<6;j++)
		{
			...
			board[this_index].color = -1;
			board[this_index].piece = 0;
			if(j == 0 || i == 0 || i == 9 || j == 5){
...
int CDC_AI::find_jump(int from, int per_step, int turn){
	...
	while(true){
		...
		if(find_pivot == false && !board[current_to].empty)
			find_pivot = true;
		else if(find_pivot && !board[current_to].empty && (board[current_to].color != turn))
			return current_to;
	}
	return 0;
}
...
void CDC_AI::expandMoves(int capture_list[65], int move_list[65], int* capture_list_index, int* move_list_index){
	(*capture_list_index) = (*move_list_index) = 0;
...
void CDC_AI::generateMove(char move[6]){
	...
	if(capture_list_index > 0){
		have_move = true;
		move_decision = capture_list[rand() % capture_list_index];
	}
	else if(move_list_index > 0){
		have_move = true;
		move_decision = move_list[rand() % move_list_index];
	}
	else{
		//flip
		for(int i = 0; i < 60; i++){
	...
	if(move[2]=='('){
		...
		if(!board[dst].empty){  //remove from plist
			...
			for(int i = 0; i < num_pieces[captured_color]; i++){
				if(piece_type == plist[captured_color][i].piece_type)
